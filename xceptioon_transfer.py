...
NUM_EPOCHS    = 20
WEIGHTS_FINAL = 'model-transfer-Chest-Xception-final.h5'
...
model = applications.Xception(weights = "imagenet", include_top=False, input_shape=[150,150,3])
...
csv_logger = CSVLogger(filename="transfer-Chest-Xception000001-log.csv"+str(int(time.time())))
#checkpointer = ModelCheckpoint(filepath='deepLearning/jamccomb/weights/xception/weights.{epoch:02d}-{val_acc:.2f}.hdf5',monitor='val_loss', verbose=1, save_best_only=True, mode='min')
cblist = [csv_logger]
